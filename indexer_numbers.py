...
class WikiHandler( xml.sax.ContentHandler ):
   ...
   def tokenizer(self,content,flag):
      ...
      if content!='':
	      ...
	      numb = pattern.split(content)
	      for l in numb:
		    match = re.match(r'([1-3][0-9]{3})', l)
		    match2 = re.match(r'(^\d{2}$)',l)
	            if match is not None:
			words.append(match.group(1))
	            if match2 is not None:
			words.append(match2.group(1))
...
def write_to_file(file_no):
	...
	global word_list
        f = open(os.path.join(fileDir,"numb_dict_file"+str(file_no)+".txt" ),'w')	
